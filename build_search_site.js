...
function judgeStore(currentPrice, recom, upper) {
  ...
  if (price <= rec * 0.9 && price <= cap) return '縦積み可';
  if (price <= rec && price <= cap) return '少数なら買い';
  if (price <= cap) return 'かなり安ければ';
  return '見送り';
}
...
function normalizeRecord(item) {
  const currentPrice = num(item.snkPrice || item.price);
  const avgPrice = num(item.price || item.snkPrice);
...
async function main() {
  ...
  const rows = source.filter((item) => item.title === 'ポケモン').map(normalizeRecord);
